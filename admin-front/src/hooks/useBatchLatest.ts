...
import type { BatchLatestInfo, BatchType } from '@/types/batch';

/**
 * 배치 주기별 최신 실행 정보를 조회하는 커스텀 훅.
 * 페이지 진입 시 1회 호출, 페이징과 무관하게 캐싱된다.
 */
export function useBatchLatest(batchType?: BatchType) {
  const { data, isLoading, isError } = useQuery<BatchLatestInfo[], Error>({
    queryKey: [...BATCH_KEYS.latest(), batchType],
    queryFn: () => fetchBatchLatest(batchType),
    staleTime: 60_000,
    retry: 3,
  });
