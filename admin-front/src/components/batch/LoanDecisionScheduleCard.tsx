...
interface LoanDecisionScheduleCardProps {
...
/**
 * 시스템 심사 배치 현황 카드
 * 대출 신청 건에 대한 시스템 심사 배치 정보 표시
 */
export default function LoanDecisionScheduleCard({ latestBatch }: LoanDecisionScheduleCardProps) {
    ...
    <div className="bg-white border border-border-default rounded-lg p-5 flex-1">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <p className="text-lg font-bold text-primary">매일 02:00</p>
        </div>
      ...
      <div className="mt-3 pt-3 border-t border-border-default">
        <p className="text-xs text-text-disabled mb-1">심사 대상</p>
        <p className="text-sm text-text-secondary">대출 신청 건 (SUBMITTED 상태)</p>
      </div>
    </div>
  );
}
